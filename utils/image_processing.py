...
def resize_image_padding(image, resize_height=None, resize_width=None):
    """
    等比例图像resize,保持原始图像内容比，避免失真,短边会0填充
    :param size:
    """
    height, width, _ = image.shape
    scale = min([resize_width / width, resize_height / height])
    new_size = [int(width * scale), int(height * scale)]
    pad_w = resize_width - new_size[0]
    pad_h = resize_height - new_size[1]
...
def draw_points_text(img, points, texts=None, color=(255, 0, 0), thickness=1, drawType="simple"):
    ...
    for point, text in zip(points, texts):
        point = (int(point[0]), int(point[1]))
        cv2.circle(img, point, thickness * 4, color, -1)
