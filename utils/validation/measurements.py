...
def compute_average_precision_per_class(num_true_cases, gt_boxes, difficult_cases,
    ...
    with open(prediction_file) as f:
        ...
        for line in f:
            ...
            box = np.asarray([float(v) for v in t[2:]], dtype=np.float32)
            box = box[np.newaxis, :]
...
def print_info(result, name, target_scores=0.5):
    ...
    print("scores:{}".format(sorted_scores[index]))
    print("ground true:{}".format(num_true_cases))
    print("true_positive:{}".format(true_positive[index]))
    print("false_positive:{}".format(false_positive[index]))
    print("tp_fp:{}".format(tp_fp[index]))

    print("recall:{}".format(recall[index]))
