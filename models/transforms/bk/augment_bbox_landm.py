        ...
        if len(boxes) > 0:
            ...
            if kwargs:
                for k in kwargs.keys():
                    nums = kwargs[k].shape[1]
                    kwargs[k][:, 0:nums:2] = np.clip(kwargs[k][:, 0:nums:2], 0, width)
                    kwargs[k][:, 1:nums:2] = np.clip(kwargs[k][:, 1:nums:2], 0, height)
        return image, boxes, labels, kwargs
...
class ToPercentCoords(object):
    ...
    def __call__(self, image, boxes=None, labels=None, **kwargs):
        if len(boxes) > 0:
            ...
            if kwargs:
                for k in kwargs.keys():
                    kscale = [width, height] * int(kwargs[k].shape[1] / 2)
                    kwargs[k] = kwargs[k] / kscale
...
class Resize(object):
    ...
    def __call__(self, image, boxes=None, labels=None, **kwargs):
        ...
        if not boxes is None and len(boxes) > 0:
            ...
            if kwargs:
                for k in kwargs.keys():
                    kscale = [self.size[0] / width, self.size[1] / height] * int(kwargs[k].shape[1] / 2)
                    kwargs[k] = kwargs[k] * kscale
...
class RandomCrop(object):
    ...
    def __call__(self, img, boxes, classes, **kwargs):
        """
        Args:
        ...
        return img, boxes, classes, kwargs
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, boxes, classes, **kwargs):
        """
        :param image:
        ...
        return image, boxes, classes, kwargs
