...
class RetinaFaceLandm(nn.Module):
    def __init__(self, prior_boxes=None, phase='train'):
        """
        :param prior_cfg:  Network related settings.
        ...
        prior_cfg = prior_boxes.prior_cfg
        self.boxes_expand = [len(boxes) * (len(self.aspect_ratios)) for boxes in self.min_sizes]  # [3,2,2,3]

        super(RetinaFaceLandm, self).__init__()
        self.phase = phase
        backbone = None
        if prior_cfg['name'] == 'mobilenet0.25':
            backbone = MobileNetV1()
            if prior_cfg['pretrain']:
        ...
        elif prior_cfg['name'] == 'Resnet50':
            import torchvision.models as models
            backbone = models.resnet50(pretrained=prior_cfg['pretrain'])

        self.body = _utils.IntermediateLayerGetter(backbone, prior_cfg['return_layers'])
        in_channels_stage2 = prior_cfg['in_channel']
        in_channels_list = [
            in_channels_stage2 * 2,
            in_channels_stage2 * 4,
            in_channels_stage2 * 8,
        ]
        out_channels = prior_cfg['out_channel']
        ...
        self.ClassHead = self._make_class_head(fpn_num=3,
                                               inchannels=prior_cfg['out_channel'],
                                               anchor_num=self.boxes_expand)
