...
class RFB(nn.Module):
    ...
    def multibox(self, num_classes):
        loc_layers = []
        conf_layers = []
        loc_layers += [depth_conv2d(64, self.boxes_expand[0] * 4, kernel=3, pad=1)]
        conf_layers += [depth_conv2d(64, self.boxes_expand[0] * num_classes, kernel=3, pad=1)]

        loc_layers += [depth_conv2d(128, self.boxes_expand[1] * 4, kernel=3, pad=1)]
        conf_layers += [depth_conv2d(128, self.boxes_expand[1] * num_classes, kernel=3, pad=1)]

        loc_layers += [depth_conv2d(256, self.boxes_expand[2] * 4, kernel=3, pad=1)]
        conf_layers += [depth_conv2d(256, self.boxes_expand[2] * num_classes, kernel=3, pad=1)]

        loc_layers += [nn.Conv2d(256, self.boxes_expand[3] * 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(256, self.boxes_expand[3] * num_classes, kernel_size=3, padding=1)]
        return nn.Sequential(*loc_layers), nn.Sequential(*conf_layers)

    def multibox_bk(self, num_classes):
        loc_layers = []
        conf_layers = []
        loc_layers += [depth_conv2d(64, 3 * 4, kernel=3, pad=1)]
        conf_layers += [depth_conv2d(64, 3 * num_classes, kernel=3, pad=1)]
    ...
    def forward(self, inputs):
        ...
        for (x, l, c) in zip(detections, self.loc, self.conf):
            loc.append(l(x).permute(0, 2, 3, 1).contiguous())
            conf.append(c(x).permute(0, 2, 3, 1).contiguous())
        # [print(d.shape) for d in loc]
        # [print(d.shape) for d in conf]
        bbox_regressions = torch.cat([o.view(o.size(0), -1, 4) for o in loc], 1)
