...
class MultiBoxLossLandm(nn.Module):
    def __init__(self, priors, neg_pos_ratio,
                 center_variance, size_variance, overlap_threshold, device):
    ...
    def forward(self, confidence, predicted_locations, predicted_landms, gt_labels, gt_locations, gt_landms):
        ...
        for i in range(len(gt_labels)):
            gt_locations[i], gt_labels[i], gt_landms[i] = self.match_prior(gt_boxes=gt_locations[i],
                                                                           gt_labels=gt_labels[i],
                                                                           gt_landms=gt_landms[i])
        gt_locations = torch.stack(gt_locations, 0).to(self.device)
        gt_labels = torch.stack(gt_labels, 0).to(self.device)
        gt_landms = torch.stack(gt_landms, 0).to(self.device)
...
class MatchPrior(object):
    ...
    def __call__(self, gt_boxes, gt_labels, gt_landms):
        ...
        landms = box_utils.encode_landm(landms, self.center_form_priors,
