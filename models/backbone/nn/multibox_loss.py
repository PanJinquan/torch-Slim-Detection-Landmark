...
class MultiboxLoss(nn.Module):
    ...
    def forward(self, confidence, predicted_locations, labels, gt_locations):
        ...
        if len(confidence) > 0:
            ...
            l1_loss = smooth_l1_loss / num_pos
            class_loss = classification_loss / num_pos
        else:
            l1_loss = torch.autograd.Variable(torch.FloatTensor([0]), requires_grad=True).to(self.device)
            class_loss = torch.autograd.Variable(torch.FloatTensor([0]), requires_grad=True).to(self.device)
        # return smooth_l1_loss / num_pos, classification_loss / num_pos
        return l1_loss, class_loss
