...
def per_class_nms(bboxes, scores, prob_threshold, iou_threshold, top_k=200, keep_top_k=100):
    """
...
def bboxes_nms(input_bboxes, input_scores, prob_threshold, iou_threshold, top_k=200, keep_top_k=100):
    ...
    dets = np.concatenate(picked_boxes_probs)
...
def per_class_landm_nms(bboxes, scores, landms, prob_threshold, iou_threshold, top_k=200, keep_top_k=100):
    """
    :param bboxes: (num_boxes, 4)
    :param scores:(num_boxes,)
    :param landms:(num_boxes, 10)
...
def bboxes_landm_nms(input_bboxes,
                     input_scores,
                     input_landms,
                     prob_threshold,
                     iou_threshold,
    ...
    if not isinstance(input_scores, np.ndarray):
        input_scores = np.asarray(input_scores)
    if not isinstance(input_landms, np.ndarray):
        input_landms = np.asarray(input_landms)
    ...
    for class_index in range(0, input_scores.shape[1]):
        ...
        if subset_probs.shape[0] == 0:
            continue
        subset_boxes = input_bboxes[index, :]
        subset_landms = input_landms[index, :]
        sub_boxes_probs, subset_landms = per_class_landm_nms(subset_boxes,
                                                             subset_probs,
                                                             subset_landms,
                                                             prob_threshold=prob_threshold,
                                                             iou_threshold=iou_threshold,
                                                             ...
                                                             keep_top_k=keep_top_k)
        picked_boxes_probs.append(sub_boxes_probs)
        picked_landms.append(subset_landms)
        picked_labels += [class_index] * sub_boxes_probs.shape[0]
